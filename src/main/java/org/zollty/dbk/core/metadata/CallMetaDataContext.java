...
package org.zollty.dbk.core.metadata;
...
import org.zollty.dbk.core.SqlReturnResultSet;
import org.zollty.dbk.core.namedparam.SqlParameterSource;
import org.zollty.dbk.core.namedparam.SqlParameterSourceUtils;
...
public class CallMetaDataContext {
			...
			if (declaredParams.containsKey(paramNameToCheck) ||
				...
				if (meta.getParameterType() == DatabaseMetaData.procedureColumnReturn) {
					if (!isFunction() && !isReturnValueRequired() &&
						...
						String returnNameToUse =(SpringUtils.hasLength(meta.getParameterName()) ?
	...
	public String createCallString() {
		...
		if (isFunction() || isReturnValueRequired()) {
			callString = "{? = call " +
					(SpringUtils.hasLength(catalogNameToUse) ? catalogNameToUse + "." : "") +
					(SpringUtils.hasLength(schemaNameToUse) ? schemaNameToUse + "." : "") +
					procedureNameToUse + "(";
			parameterCount = -1;
		}
		else {
